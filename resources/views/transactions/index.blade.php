        ...
        <tbody>
            @forelse($transactions as $index => $t)
                <tr>
                    ...
                    <td>{{ $t->occurred_at ? $t->occurred_at->format('d M Y') : '-' }}</td>
                    <td>
                        @if($t->bukti)
                            @php $ext = pathinfo($t->bukti, PATHINFO_EXTENSION); @endphp
                            @if(in_array(strtolower($ext), ['jpg','jpeg','png','gif','webp']))
                                <a href="{{ Storage::url($t->bukti) }}" target="_blank">
                                    <img src="{{ Storage::url($t->bukti) }}" alt="Bukti" style="max-height:60px; border-radius:4px;">
                                </a>
                            @else
                                <a href="{{ Storage::url($t->bukti) }}" target="_blank">Lihat Bukti</a>
                            @endif
                        @else
                            -
                        @endif
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    const options = {
        ...
        title: { text: 'Statistik Transaksi Bulanan', align: 'left' }
    };
